...
{    
    ...
    vec4 n = texture(textUnity, floor(uv * ntiles) / ntiles);
...
void main()
{
    ...
    float sound = texture(textUnity, vec2(0.01, 1.0 - r)).r;
    sound = pow(sound, 1.5);
    ...
    vec3 c  = mix(c1, c2, r);
    vec3 coltunnel =  sound * c * tex(uv2, sound) + ( 0.15 * texture(textUnity, uv2).rgb);
